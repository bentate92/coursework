...
int algo3(vector<int>& theNumbers, int startIdx, int endIdx, struct values& sums) {
	...
	vector<int> left;
	vector<int> right;
	for(int i = 0; i < distance(theNumbers.begin(), theNumbers.begin() + mid); i++) {
		left.push_back(theNumbers[i]);
	}
	for(int i = distance(theNumbers.begin(), theNumbers.begin() + mid); i < n; i++) {
		right.push_back(theNumbers[i]);
	}
