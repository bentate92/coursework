/* CS261- Assignment 7 - DFS and BFS implementation (graph.c)*/
/* Name: Benjamin Tate
 * Date: 6/5/2016
 * Description: Implementation of depth-first and breadth-first
 *    graph traversals
 */
...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
  ...
  /*Create deque to be used as a stack*/
  Deque* stack = dequeNew();

  /*Add start vertex to stack*/
  dequePushBack(stack, source);

  /*Continue until all reachable vertices have been traversed*/
  while(!dequeIsEmpty(stack)) {
    /*Create a vertex pointer to iterate through graph*/
    Vertex* itr;
    
    /*Assign itr the top of the stack and remove from stack*/
    itr = dequeBack(stack);
    dequePopBack(stack);

    /*Return 1 if destination has been reached*/
    if(itr == destination) {
      return 1;
    }

    /*If itr has not been reached before*/
    if(itr->isVisited != 1) {
      /*Update isVisited*/
      itr->isVisited = 1;

      /*Add itr's neighbors to the stack*/
      for(int i = 0; i < itr->numNeighbors; i++) {
        dequePushBack(stack, itr->neighbors[i]);
      }
    }
  }
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
  ...
  /*Create deque to be used as a queue*/
  ...
  while(!dequeIsEmpty(queue)) {
    ...
    /*Assign itr the front of the queue and remove from queue*/
    itr = dequeFront(queue);
    dequePopFront(queue);
    ...
    if(itr->isVisited != 1) {
      ...
      /*Add itr's neighbors to the queue*/
      for(int i = 0; i < itr->numNeighbors; i++) {
        dequePushBack(queue, itr->neighbors[i]);
      }
    }
  }
